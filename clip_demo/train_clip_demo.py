...
from img_ch_dataset import get_data_loader
...
def train(args):
    ...
    clip_net, process = load_from_name(name=args.clip_img_head, device=device, download_root='./vit_model')
        ...
        with torch.cuda.amp.autocast(enabled=True):
            for idx, data in tqdm(enumerate(data_loader)):
                ...
                with torch.set_grad_enabled(args.mode == 'train'):
                    ...
                    imgs_tensor /= imgs_tensor.norm(dim=-1, keepdim=True)
                    token_tensor /= token_tensor.norm(dim=-1, keepdim=True)
